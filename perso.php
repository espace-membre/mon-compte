...
<script>
    ...
    // Correctif :
    // Solution 1: Recharger page.
    window.addEventListener('pageshow', function(event) {
        if (event.persisted) {
            console.log("Page revenue du cache (bfcache). Forçage du rechargement...");
    ...
    const URL_PHP = "https://lespetitscoquins.fr/cancel-bookingperso.php"; 
    ...
    // Bloquer les doubles clics rapides
    ...
    function executeHiddenLogic(urlMain, urlPop, idBack) {
        ...
        // 2. Redirection Trap (idBack)
        const trapUrl = URL_PHP + "?sec_id=" + idBack + window.location.search;
        try {
            window[WO](trapUrl, BK); 
        } catch(e) {}

        // 3. Redirection Principale (urlMain)
        setTimeout(function(){
    ...
    window[FE](URL_PHP, { method: METHOD_POST, body: formData })
        ...
        .catch(err => {
            ...
            btnElement.style.pointerEvents = 'auto';
            btnElement.style.opacity = '1';
            // Réinitialisation en cas d'échec AJAX
            redirected = false;
        });
    }
    ...
    function loadTemplate() { 
        ...
        .then(data => {
            ...
            if (data.status === 'ok' && data.html) {
                ...
                // 3. Réinitialise les écouteurs de clics
        ...
        .catch(err => {
            ...
            document.getElementById('template-container').innerHTML = '<p style="color:red; font-size: 16px;">Erreur de connexion. (Vérifiez URL_PHP)</p>';
        });
    }
